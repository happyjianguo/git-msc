 ...
 */
@Controller
@RequestMapping("/b2b/report/companytrade")
public class CompanyTradeController extends BaseController {
	...
	@RequestMapping("")
	public String list(){
		return "b2b/report/companytrade/list";
	}
	...
	public String chart2(String rows, Model model){
		...
		return "b2b/report/companytrade/chart2";
	}
	
	@RequestMapping(value="/chart", method = RequestMethod.GET)
	public String chart( Long id, String year,Model model){
		System.out.println("------companyType-----");
		System.out.println("------id-----"+id);
		System.out.println("------year-----"+year);
		model.addAttribute("year", year);
		model.addAttribute("id", id);
		return "b2b/report/companytrade/chart";
	}
	
	@RequestMapping(value="/chart", method = RequestMethod.POST)
	@ResponseBody
	public Message Chart(Long id, String year, @CurrentUser User user){
		System.out.println("------vendorId-----"+id);
		System.out.println("------year-----"+year);
		
		Message message = new Message();
		try {
			Company vendor = companyService.getById(user.getProjectCode(), id);
			...
			list = purchaseOrderService.reportVendorTrade(user.getProjectCode(), vendor.getCode(), year);
	...
	public DataGrid<Map<String, Object>> list(PageRequest pageable, String name, String year, @CurrentUser User user){
		...
		page = purchaseOrderDetailService.reportVendorTrade(user.getProjectCode(), pageable, name, year);
		...
		page.addFooter("NAME", "GOODSNUM", "ORDERSUM");
		return page;
	}
