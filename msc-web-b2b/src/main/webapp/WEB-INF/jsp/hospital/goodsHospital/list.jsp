...
<shiro:hasPermission name="hospital:goodsHospital:export">
    <a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-export'" onclick="exportexcel()">导出</a>
</shiro:hasPermission>
        </div>
		<table  id="dg" ></table>
	</div>
	...
	$('#dg').datagrid({
		...
		columns:[[
		   		...
		   		<%--toolbar: [{--%>
					...
					<%--text:"导出",--%>
					<%--handler: function() {--%>
                        <%--window.open(" <c:out value='${pageContext.request.contextPath }'/>/hospital/goodsHospital/export.htmlx");--%>
                    <%--}--%>
			<%--}]--%>
	});
