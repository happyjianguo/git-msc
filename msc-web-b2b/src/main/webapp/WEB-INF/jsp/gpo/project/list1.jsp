...
$(function(){
	...
	$('#dg').datagrid({
		...
		columns:[[
		        	...
		        	{field:'endMonthDef',title:'默认计划开始时间',width:10,align:'center'},
	...
	$('#dg1').datagrid({
		...
		columns:[[
        	...
        	{field:'directory.rcDosageFormName',title:'推荐剂型',width:10,align:'center',formatter: function(value,row,index){
        		return row.directory.rcDosageFormName;
        	}},
        	...
        	{field:'directory.producerNames',title:'生产厂家',width:10,align:'center',formatter: function(value,row,index){
        		return row.directory.producerNames;
        	}},
        	{field:'directory.note',title:'备注',width:10,align:'center',formatter: function(value,row,index){
        		return row.directory.note;
        	}},
        	{field:'num',title:'报量数',width:10,align:'center',formatter: function(value,row,index){
        		return row.num;
        	}},
        	{field:'hospitalNum',title:'报量医院数',width:10,align:'center',formatter: function(value,row,index){
        		if(row.hospitalNum== null || row.hospitalNum == 0)
        			return "";
        		else
        			return "<a class='dgbtn1' href='#' onclick='hospital1Open("+row.id+")' class='easyui-linkbutton'>"+row.hospitalNum+"</a>";
        	}}
   		]],
		toolbar: [{
			iconCls: 'icon-add',
...
//折页1窗口定标
function hospital1Open(projectDetailId) {
	if(detailRow.id == null) return;
	top.$.modalDialog({
		title : "报量医院",
		width : 1000,
		...
		onLoad:function(){
			var f = top.$.modalDialog.handler.find("#projectDetailId").val(projectDetailId);
			top.search();
		},
		onClose:function() {
			top.$.modalDialog.handler.dialog('destroy');
