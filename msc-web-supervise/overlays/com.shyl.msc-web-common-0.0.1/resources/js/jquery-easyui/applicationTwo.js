...
$.app = {
     ...
     * @param settings
     */
    modalDialogTwoTwo : function(title, url, settings) {
        ...
        var defaultSettings = {
            ...
            _close : function(modal) {
                $(modal).dialog("close");
                if($.app._modalDialogTwoQueue.length > 0) {
                    $.app._modalDialogTwoQueue.pop();
                }
            },
            buttons:{
                '确定': function() {
        ...
        $.ajax({
            ...
            headers: { table:true }
                ...
                if(!$.app._modalDialogTwoQueue) {
                    $.app._modalDialogTwoQueue = new Array();
                }
                $.app._modalDialogTwoQueue.push(dialog);
...
//            });
    ...
    cancelModelDialog : function() {
        if($.app._modalDialogTwoQueue && $.app._modalDialogTwoQueue.length > 0) {
            $.app._modalDialogTwoQueue.pop().dialog("close");
        }
    }
    ,
...
$.parentchild = {
    ...
    updateChild : function($tr, updateUrl, modalSettings) {
        ...
        $.app.modalDialogTwo("修改", updateUrl, modalSettings);
    }
    ,
    /**
     * 以当前行复制一份
    ...
    copyChild : function($tr, updateUrl, modalSettings) {
        ...
        updateUrl = updateUrl.replace("{trId}", $tr.prop("id"));
        $.app.modalDialogTwo("复制", updateUrl, modalSettings);
    }
    ,
    /**
     * 删除子
    ...
    initParentForm : function(options) {
        ...
        $(".btn-create-child").click(function() {
            $.app.modalDialogTwo("新增", options.createUrl, options.modalSettings);
        });
