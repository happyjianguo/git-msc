...
<body class="easyui-layout">
	<div data-options="region:'center',title:''">
		...
		<div id="toolbar" class="search-bar" >
			<shiro:hasPermission name="supervise:hisInAbsDrugRateAnalysis:export">
...
<script>
	...
	$(function() {
		...
		$('#dg').datagrid({
			...
			pagination : true,
			url : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInAbsDrugRateAnalysis/page.htmlx",
	...
	function addOpen(row) {
		...
		var title = "";
		query["queryType"] = window.nextQueryType;
		var url = "/supervise/hisInAbsDrugRateAnalysis/index.htmlx?queryType=" + window.nextQueryType + "&jsonStr=";
	...
	function getColumns(queryType) {
		var columns = [ {
			...
			field : 'RATEABSNUM',
			title : '<span title="住院抗菌药物使用人次  / 出院人次" class="tip">住院抗菌药物使用率(%)</span>',
			width : 120,
			align : 'center'
		} ];
	...
	function showChart() {
		...
		top.$.modalDialog({
			...
			width : 800,
			height : 460,
			href : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInAbsDrugRateAnalysis/chart.htmlx"
		});
	}
	
	function exportexcel(){
		var url = "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInAbsDrugRateAnalysis/export.htmlx?";
		for (var k in window.queryParams) {
