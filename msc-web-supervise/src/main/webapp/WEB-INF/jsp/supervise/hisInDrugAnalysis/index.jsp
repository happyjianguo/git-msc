...
<body class="easyui-layout">
	<div data-options="region:'center',title:''">
		<div id="toolbar" class="search-bar" >
				...
				<shiro:hasPermission name="supervise:hisInDrugAnalysis:export">
...
<script>
	...
	$(function() {
		...
		$('#dg').datagrid({
			...
			pagination : true,
			url : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInDrugAnalysis/page.htmlx",
	...
	function openSearch() {
		top.$.modalDialog({
			...
			width : 600,
			height : 360,
			href : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInDrugAnalysis/search.htmlx",
			onLoad : function() {
			...
			buttons : [ {
				...
				handler : function() {
					...
					try {
						...
						/* var orgLevel = f.find("#orgLevel").combobox("getValue");
	...
	function openChild(row) {
		...
		var url = "/supervise/hisInDrugAnalysis/index.htmlx?queryType=" + window.nextQueryType + "&jsonStr=";
	...
	function getColumns(queryType) {
		var columns = [ {
			...
			field : 'AVGSUM',
			title : '<span title="出院病人总费用  / 出院病人人次" class="tip">每住院人均费用</span>',
			width : 150,
			align : 'center'
		}, {
			field : 'DRUGSUM',
			...
			field : 'RATEDRUGSUM',
			title : '<span title="出院病人药品总费用  / 出院病人总费用" class="tip">药品费用占比(%)</span>',
			width : 150,
			align : 'center'
		}, {
			field : 'OUTNUM',
			...
			field : 'AVGDAYDRUGSUM',
			title : '<span title="住院病人药品总费用  / 住院床日数" class="tip">每住院床日药品费用</span>',
			width : 150,
			align : 'center'
		}, {
			field : 'DRUGNUM',
			...
			field : 'AVGDRUGNUM',
			title : '<span title="住院药品品规数  / 出院病人人次" class="tip">每住院人均品规数</span>',
			width : 150,
			align : 'center'
		} ];
		var first = {
	...
	function showChart() {
		...
		top.$.modalDialog({
			...
			width : 800,
			height : 460,
			href : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInDrugAnalysis/chart.htmlx"
		});
	}
	...
	function exportexcel(){
		...
		var url = "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInDrugAnalysis/export.htmlx?";
		for ( var k in window.queryParams) {
			url += "&" +encodeURIComponent(k) + "=" + encodeURIComponent(window.queryParams[k]);
		}
		console.log(JSON.stringify(window.queryParams));
		console.log(url);
		window.open(url);
	}
</script>