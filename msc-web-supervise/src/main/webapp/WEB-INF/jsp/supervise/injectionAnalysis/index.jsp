	...
	<div data-options="region:'center',title:''" >
		...
		<span id = "search1">
        	...
        	<div id="toolbar" class="search-bar" >
				<shiro:hasPermission name="supervise:injectionAnalysis:export">
...
$(function(){
	...
	$("#dg").datagrid({
		...
		pagination:true,
		url:" <c:out value='${pageContext.request.contextPath }'/>/supervise/injectionanalysis/page.htmlx",
...
function addOpen(row) {
	...
	var title = "";
	query["queryType"] = window.nextQueryType;
	var url = "/supervise/injectionanalysis.htmlx?queryType="+window.nextQueryType+"&jsonStr=";
	if (window.nextQueryType == 1) {
		...
		title = "门诊注射剂使用分析(门诊处方)";
	}
    top.addTab(title, url,null,true);
}	
function getColumns(queryType) {
	var columns = [
			{field:'RECIPENUM',title:'总处方数',width:120,align:'center'}, 
			{field:'INJECTIONNUM',title:'注射剂处方数',width:120,align:'center'},
			{field:'INJECTIONPROPORTION',title:'<span title=" 注射剂处方数  / 同期总处方数 " class="tip">注射剂使用处方比例(%)</span>',width:150,align:'center'},
			{field:'VISITORNUM',title:'总就诊人次',width:120,align:'center'}, 
			{field:'INTRAINJECTIONNUM',title:'静脉输液人次',width:120,align:'center'}, 
			{field:'INTRANPROPORTION',title:'<span title=" 静脉输液人次  / 同期总就诊人数次" class="tip">静脉输液处方比例(%)</span>',width:160,align:'center'}
		   ];

	var first = {field:'ZONENAME',title:'区域',width:120,align:'center',formatter: function(value,row,index){
...
function showChart() {
	...
	top.$.modalDialog({
		...
		width : 800,
		height : 460,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/injectionanalysis/chart.htmlx"
	});
}
...
function exportexcel(){
	...
	var url = " <c:out value='${pageContext.request.contextPath }'/>/supervise/injectionanalysis/export.htmlx?";
	for (var k in window.queryParams) {
