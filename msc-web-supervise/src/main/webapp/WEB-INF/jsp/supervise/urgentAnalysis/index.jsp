...
<body class="easyui-layout">
	<div data-options="region:'center',title:''" >
		<div id="toolbar" class="search-bar" >
				...
				<shiro:hasPermission name="supervise:urgentAnalysis:export">
...
$(function(){
	...
	var toolbar = [{
		...
		handler: function(){
			...
			var url = " <c:out value='${pageContext.request.contextPath }'/>/supervise/urgentAnalysis/export.htmlx?";
	...
	$("#dg").datagrid({
		...
		pagination:true,
		url:" <c:out value='${pageContext.request.contextPath }'/>/supervise/urgentAnalysis/page.htmlx",
		...
		onLoadSuccess:function(data){
			var array = data.rows;
			var begindata = array[0]['beginDate'];
			var enddata = array[0]['endDate'];
			var timetile="";
...
function openSearch() {
	top.$.modalDialog({
		...
		width : 600,
		height : 360,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/urgentAnalysis/search.htmlx",
		onLoad:function(){
...
function addOpen(row) {
	...
	var url = "/supervise/urgentAnalysis.htmlx?queryType="+window.nextQueryType+"&jsonStr=";
...
function getColumns(queryType) {
	var columns = [
    	{field:'VISITORNUM',title:'总就诊人次',width:100,align:'center'},
	...
	var first = {field:'ZONENAME',title:'区域',width:100,align:'center',formatter: function(value,row,index){
...
function showChart() {
	...
	top.$.modalDialog({
		...
		width : 800,
		height : 460,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/urgentAnalysis/chart.htmlx"
	});
}
