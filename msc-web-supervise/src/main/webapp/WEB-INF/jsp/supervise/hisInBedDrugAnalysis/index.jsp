...
<body class="easyui-layout">
	<div data-options="region:'center',title:''">
		...
		<div id="toolbar" class="search-bar" >
				<shiro:hasPermission name="supervise:hisInBedDrugAnalysis:export">
...
<script>
	...
	$(function() {
		...
		$('#dg').datagrid({
			...
			pagination : true,
			url : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInBedDrugAnalysis/page.htmlx",
	...
	function addOpen(row) {
		...
		var title = "";
		query["queryType"] = window.nextQueryType;
		var url = "/supervise/hisInBedDrugAnalysis/index.htmlx?queryType=" + window.nextQueryType + "&jsonStr=";
	...
	function getColumns(queryType) {
		var columns = [ {
			...
			field : 'AVGDAYDRUGSUM',
			title : '<span title="出院病人药品总费用  / 出院病人总住院天数" class="tip">每住院床日药品费用</span>',
			width : 120,
			align : 'center'
		} ];
	...
	function showChart() {
		...
		top.$.modalDialog({
			...
			width : 800,
			height : 460,
			href : "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInBedDrugAnalysis/chart.htmlx"
		});
	}
	
	function exportexcel(){
		var url = "<c:out value='${pageContext.request.contextPath }'/>/supervise/hisInBedDrugAnalysis/export.htmlx?";
		for (var k in window.queryParams) {
