...
<body class="easyui-layout" >
			<div id="toolbar" class="search-bar" >
				...
				<shiro:hasPermission name="supervise:auxiliaryDrug:export">
        		...
        		<shiro:hasPermission   name="supervise:auxiliaryDrug:add">
        			<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-chart_bar',plain:true" onclick="showProduct()">辅助用药设置</a>
        		</shiro:hasPermission >
		        <span id='time'></span>
	    	</div>
...
$(function(){
	...
	$('#dg').datagrid({
		...
		pagination:true,
		url:" <c:out value='${pageContext.request.contextPath }'/>/supervise/auxiliaryDrug/page.htmlx",
...
function openSearch() {
	top.$.modalDialog({
		...
		width : 600,
		height : 360,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/auxiliaryDrug/search.htmlx",
		onLoad:function(){
...
function openChild(row) {
	...
	var url = "/supervise/auxiliaryDrug/index.htmlx?queryType="+window.nextQueryType+"&jsonStr=";
...
function showChart() {
	...
	top.$.modalDialog({
		...
		width : 800,
		height : 460,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/auxiliaryDrug/chart.htmlx"
	});
}
function showProduct() {
	...
	top.$.modalDialog({
		...
		width : 800,
		height : 460,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/medicine.htmlx",
		buttons : [{
			text : '取消',
			iconCls : 'icon-cancel',
			handler : function() {
...
function exportexcel(){
	...
	var url = " <c:out value='${pageContext.request.contextPath }'/>/supervise/auxiliaryDrug/export.htmlx?";
	for (var k in window.queryParams) {
