...
<body class="easyui-layout">
	<div data-options="region:'center',title:''" >
		...
		<span id = "search1">
        	...
        	<a class="easyui-linkbutton" data-options="iconCls:'icon-search'"  onclick="query_btn()">查询</a>
        	</form>
        	<div id="toolbar" class="search-bar" >
			<shiro:hasPermission name="supervise:hisDrugIncomeRatio:export">
...
$(function(){
	...
	$("#dg").datagrid({
		...
		pagination:true,
		url:" <c:out value='${pageContext.request.contextPath }'/>/supervise/hisDrugIncomeRatio/page.htmlx",
...
function addOpen(row) {
	...
	var title = "";
	query["queryType"] = window.nextQueryType;
	var url = "/supervise/hisDrugIncomeRatio.htmlx?queryType="+window.nextQueryType+"&jsonStr=";
	if (window.nextQueryType == 1) {
        ...
        query["queryType"] = window.nextQueryType-1;
        query["query['provinceCode_S_EQ']"] = undefined;
        query["query['cityCode_S_EQ']"] = undefined;
        query["query['countyCode_S_EQ']"] = undefined;
        query["query['departCode_S_EQ']"] = row.DEPARTCODE;
	...
	if(window.nextQueryType <= 5&&window.nextQueryType >= 0){
...
function getColumns(queryType) {
	var columns = [
			{field:'OTHERSUM',title:'医疗总收入',width:150,align:'center'}, 
			{field:'DRUGSUM',title:'药品总收入',width:150,align:'center'},
			{field:'SUM',title:'医院总收入',width:150,align:'center'},
			{field:'OTHERRATIO',title:'<span title=" 药品总收入  / 同期医疗总收入 " class="tip">药品与医疗总收入比(%)</span>',width:150,align:'center',
				formatter: function(value,row,index){
					if(row.DRUGSUM.length==0){
						return null;
					}else{
						return (parseFloat(row.DRUGSUM/row.OTHERSUM)*100).toFixed(1);
					}
					/* return (parseFloat(row.ALLDRUGSUM/row.ALLOTHERSUM)*100).toFixed(1); */
				}},
			{field:'DRUGRATIO',title:'<span title=" 药品总收入  / 同期医院总收入 " class="tip">药品与总收入比(%)</span>',width:150,align:'center',
				formatter: function(value,row,index){
	...
	var two= null;
	if (queryType == 1) {
		first = {field:'PROVINCENAME',title:'省',width:150,align:'center'};
	} else if (queryType == 2) {
		first = {field:'CITYNAME',title:'市',width:150,align:'center'};
	} else if (queryType == 3) {
		first = {field:'COUNTYNAME',title:'区/县',width:150,align:'center'};
	} else if (queryType == 4) {
		first = {field:'HOSPITALNAME',title:'医院',width:200,align:'center'};
	} else if (queryType == 5) {
		first = {field:'HOSPITALNAME',title:'医院',width:200,align:'center'};
		two = {field:'MONTH',title:'月份',width:150,align:'center'};
	} 
	if(queryType == 5){
		columns.splice(0,0,first);
		columns.splice(1,0,two);
	}else{
		columns.splice(0,0,first);
	}	
	return columns;
...
function showChart() {
	...
	top.$.modalDialog({
		...
		width : 800,
		height : 460,
		href : " <c:out value='${pageContext.request.contextPath }'/>/supervise/hisDrugIncomeRatio/chart.htmlx"
	});
}
...
function exportexcel(){
	...
	var url = " <c:out value='${pageContext.request.contextPath }'/>/supervise/hisDrugIncomeRatio/export.htmlx?";
	for (var k in window.queryParams) {
