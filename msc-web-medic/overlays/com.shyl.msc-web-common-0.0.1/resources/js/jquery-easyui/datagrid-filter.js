	...
	function resizeFilter(target, field, width){
		...
		ff.each(function(){
			...
			if (width != undefined){
				...
				this.filter.resize(this, col.width - btn._outerWidth()-2);//-2 对齐补丁
			}
		});
	}
	...
	function init(target, filters){
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				//字段类型
				var fieldType = "S";
				if(fopts!=null && fopts.fieldType){
					fieldType = fopts.fieldType;
				}
				input.attr("fieldType",fieldType);
				
				//是否关闭搜索
				var isDisabled = 0;
				if(fopts!=null && fopts.isDisabled){
					isDisabled = 1
				}
				input.attr("isDisabled",isDisabled);
				...
				if (fopts && type != 'text'){
					...
					//关闭搜索
					if(input.attr("isDisabled")==1){
						input.attr("disabled",true);
						input.val("--");
						//input.hide();
						//input.css("backgroundColor","#e6e6e6");
						input.css("width","100");
					}
		...
		function createFilterButton(container, operators){
			...
			menu.menu({
				...
				onClick:function(item){
					...
					var fieldType = $(input[0]).attr("fieldType");
