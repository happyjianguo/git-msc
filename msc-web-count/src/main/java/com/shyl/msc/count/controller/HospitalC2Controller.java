...
@Controller
@RequestMapping("/count/hospitalC2")
public class HospitalC2Controller extends BaseController {
	...
	public DataGrid<HospitalC> page(PageRequest pageable,@CurrentUser User user){
		...
		for (int i = 0; i < page.getRows().size(); i++) {
			...
			Map<String, Object> m = productCService.countByMonth(hc.getMonth());
			hc.setSegmentBD(new BigDecimal(m.get("COUNT")+""));
			if(new BigDecimal(m.get("COUNT")+"").compareTo(new BigDecimal("0")) != 0){
				String per = hc.getPurchaseSpecCount()*100.00/Integer.parseInt(m.get("COUNT")+"")+"%";
				hc.setSegmentStr(per);
			}else{
				hc.setSegmentStr("0.0%");
			}
	...
	public String chart1(String hospitalCode,String hospitalName,Model model,@CurrentUser User user){
		...
		return "count/hospitalC2/chart1";
	}
	...
	public String chart2(String month,Model model,@CurrentUser User user){
		...
		return "count/hospitalC2/chart2";
	}
