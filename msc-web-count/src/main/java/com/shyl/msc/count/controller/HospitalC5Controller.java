...
@Controller
@RequestMapping("/count/hospitalC5")
public class HospitalC5Controller extends BaseController {
	...
	public DataGrid<HospitalC> page(PageRequest pageable,@CurrentUser User user){
		...
		for (int i = 0; i < page.getRows().size(); i++) {
			...
			hc.setSegmentStr("0.00%");
			if(hc.getDeliveryNum().compareTo(new BigDecimal("0"))!=0){
				//验收完成率
				BigDecimal b = hc.getInOutBoundNum().multiply(new BigDecimal("100")).divide(hc.getDeliveryNum(),2,BigDecimal.ROUND_HALF_UP);
				hc.setSegmentStr(b+"%");
			}
		}
	...
	public String chart1(String hospitalCode,String hospitalName,Model model,@CurrentUser User user){
		...
		return "count/hospitalC5/chart1";
	}
	...
	public String chart2(String month,Model model,@CurrentUser User user){
		...
		return "count/hospitalC5/chart2";
	}
