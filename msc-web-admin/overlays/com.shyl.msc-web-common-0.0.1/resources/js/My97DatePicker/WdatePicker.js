	...
	function I(G, _) {
		...
		function E(F, E) {
			...
			if (typeof(HOST_DOMAIN)!='undefined'){
				if (HOST_DOMAIN!=""){
					domain = "document.domain='"+HOST_DOMAIN+"';";
				}
			}
		
		    var iframe = document.createElement("iframe");
		    iframe.src="javascript:void((function(){document.open();" + domain + "document.write('');document.close()})())";
		    iframe.width = 97;
    		iframe.height = 9;
		    iframe.scrolling = "no";
		    iframe.id = "_iframe_wdatepicker";
		    iframe.setAttribute('frameborder', '0' , 0); 
		    iframe.setAttribute('hideFocus', 'true' , 0); 
			...
			if (iframe.attachEvent){ // IE
	       		iframe.attachEvent("onload", function(){
					...
					F.lang = H.name;
					F.skin = E.skin;
					var G = ["<head><script>", domain + "var $d, $dp, $cfg=document.cfg, $pdp = parent.$dp, $dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera, $ny, $cMark = false;", "if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};for(var p in $cfg){$dp[p]=$cfg[p];}", "document.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,1);showB($d.qsDivSel);}catch(e){};return false;};", "</script><script src=", A, "lang/", H.name, ".js charset=", H.charset, "></script>"];
       		  ...
       		  } else if (iframe.addEventListener){ // OTHER
